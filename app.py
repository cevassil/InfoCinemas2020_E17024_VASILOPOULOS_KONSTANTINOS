...
def index():
	...
	if session['loggedin'] == True:
		movie = movies.find({})
		return render_template("index.html", mail=session['email'], mov=movie)
	else:
		return redirect(url_for('login'))
#Ticket buy function
...
def buy():
	if session['loggedin'] == True:
		movie = movies.find({})
		return render_template("buy.html", mail=session['email'], mov=movie)
...
def search():
	if session['loggedin'] == True:
		if request.method== "GET" :			
			...
			movresult = movies.find({"title": msearch})
...
if __name__ == "__main__":
	...
	if not 'movies' in db.list_collection_names():
		with open('movies.json') as d:
			movie = json.load(d)	
